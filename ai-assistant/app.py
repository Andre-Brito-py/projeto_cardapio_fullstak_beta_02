...
# Importar integração de demonstração
from demo_integration import handle_demo_integration, get_demo_starters
...
async def set_starters() -> list[str]:
    ...
    """
    # Combinar starters normais com starters de demonstração
    normal_starters = await select_starter()
    demo_starters = get_demo_starters()
    return normal_starters + demo_starters
...
async def on_message(user_message: cl.Message) -> None:
    ...
    # Verificar se é uma solicitação de demonstração
    demo_handled = await handle_demo_integration(user_message, model_name)
    
    # Se não foi tratado pela demonstração, processar normalmente
    if not demo_handled:
